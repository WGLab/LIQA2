...
def convert_to_gtf(metageneStructureInformationNovel, output_file, gtf_df = None, num_cores=1):
    def update_annotation_gene(geneID, gtf_df, geneStructureInformationwNovel):
        ...
        if gtf_df_sub.shape[0] > 0:
            ...
            new_row_gene = [[geneInfo['geneChr'], 'LIQA2', 'gene', geneInfo['geneStart']+1, geneInfo['geneEnd'], '.',
        ...
        for isoform_name, exon_indices in isoformInfo.items():
            ...
            if gtf_df_sub[gtf_df_sub['attribute'].str.contains(f'transcript_id "{isoform_name}"', regex=False)].shape[0] == 0:
                new_rows_isoform = [geneInfo['geneChr'], 'LIQA2', 'transcript', isoform_start+1, isoform_end, '.',
                                    geneInfo['geneStrand'], '.',
...
def summarise_annotation(target):
    ...
    for reference_folder in reference_folders:
        ...
        output_gtf = os.path.join(reference_folder, "LIQA2_updated_annotation.gtf")
        ...
        file_names_gtf = [os.path.join(target, 'reference', f) for f in
                          os.listdir(os.path.join(target, "reference")) if
                          re.match(r'gene_annotations_liqa_\d+\.gtf', f)]
...
class ReadMapper:
    def __init__(self, target:list, bam_path:list, lowest_match=0.2, lowest_match1 = 0.8, small_exon_threshold = 20,
        ...
        self.annotation_path_gtf_novel_list = [os.path.join(target_, "reference/gene_annotations_liqa_.gtf") for target_ in target]
    ...
    def map_reads_allgenes(self, cover_existing = True, total_jobs = 1, current_job_index = 0):
        ...
        if cover_existing:
            print('If there are existing compatible matrix files, LIQA2 will overwrite them')
            genes_existing = []
        else:
            print('If there are existing compatible matrix files, LIQA2 will not overwrite them')
            if self.parse:
